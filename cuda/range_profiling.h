...
class CuptiRangeProfilerHost
{
public:
    CuptiRangeProfilerHost() = default;
    ~CuptiRangeProfilerHost() = default;

    void SetUp(std::string chipName, std::vector<uint8_t>& counterAvailibilityImage);
    void TearDown();

    CUptiResult GetMetricProperties(const std::string& metricName, CUpti_MetricType& metricType, std::string& dimUnit, std::string& hwUnit);
    ...
    void PrintProfilerRanges(double& totalNsDuration);
...
using CuptiProfilerHostPtr = std::shared_ptr<CuptiRangeProfilerHost>;
...
inline void CuptiRangeProfilerHost::SetUp(std::string chipName, std::vector<uint8_t>& counterAvailibilityImage)
{
    m_chipName = chipName;
    CUPTI_API_CALL(Initialize(counterAvailibilityImage));
}

inline void CuptiRangeProfilerHost::TearDown()
{
    CUPTI_API_CALL(Deinitialize());
}

inline CUptiResult CuptiRangeProfilerHost::GetMetricProperties(const std::string& metricName, CUpti_MetricType& metricType, std::string& dimUnit, std::string& hwUnit)
{
    ...
    dimUnit = getMetricPropertiesParams.pDimUnit;
    hwUnit = getMetricPropertiesParams.pHwUnit;
    return CUPTI_SUCCESS;
}

inline CUptiResult CuptiRangeProfilerHost::CreateConfigImage(
    std::vector<const char*> metricsList,
    std::vector<uint8_t>& configImage
)
{
...
inline CUptiResult CuptiRangeProfilerHost::EvaluateCounterData(
    size_t rangeIndex,
    std::vector<const char*> metricsList,
    std::vector<uint8_t>& counterDataImage
)
...
CUptiResult CuptiRangeProfilerHost::GetNumOfRanges(
    std::vector<uint8_t>& counterDataImage,
    size_t& numOfRanges
)
{
...
inline void CuptiRangeProfilerHost::PrintProfilerRanges(double& totalNsDuration)
{
    std::cout << "Total num of Ranges: " << m_profilerRanges.size() << "\n\n";
    std::string dimUnit;
    std::string hwUnit;
    CUpti_MetricType metricType;
    std::string metricTypeName;
    ...
    for(size_t rangeIndex = 0; rangeIndex < m_profilerRanges.size(); ++rangeIndex)
    {
        ...
        {
            if (rangeIndex == 0){
                CUPTI_API_CALL(GetMetricProperties(metric.first, metricType, dimUnit, hwUnit));
                if (metricType == CUpti_MetricType::CUPTI_METRIC_TYPE_COUNTER)
                    metricTypeName = "Counter";
                else if (metricType == CUpti_MetricType::CUPTI_METRIC_TYPE_RATIO)
                    metricTypeName = "Ratio";
                else
                    metricTypeName = "Throughput";
            }
            std::cout << std::fixed << std::setprecision(3);
            std::cout << std::setw(50) << std::left << metric.first;
            std::cout << std::setw(30) << std::right << metric.second << "\n";
            std::cout << std::setw(30) << std::left << metricTypeName << "\n";
            std::cout << std::setw(20) << std::left << dimUnit << "\n";
            std::cout << std::setw(20) << std::left << hwUnit << "\n";
            if (metric.first.find("gpu__time_duration") != std::string::npos){
                totalNsDuration += metric.second;
            }
        }
        std::cout << "-----------------------------------------------------------------------------------\n\n";
    }
...
inline CUptiResult CuptiRangeProfilerHost::Initialize(std::vector<uint8_t>& counterAvailibilityImage)
{
...
inline CUptiResult CuptiRangeProfilerHost::Deinitialize()
{
